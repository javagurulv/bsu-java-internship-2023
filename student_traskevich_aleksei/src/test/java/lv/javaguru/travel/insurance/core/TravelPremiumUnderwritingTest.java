...
public class TravelPremiumUnderwritingTest {
    ...
    void shouldReturnResponseWithCorrectAgreementPrice() {
        ...
        when(request.getAgreementDateTo()).thenReturn(createDate("10.01.2022"));
        when(dateTimeService.getDaysBetween(request.getAgreementDateFrom(), request.getAgreementDateTo()))
                .thenReturn(9L);
        BigDecimal calculatePremium = underwriting.calculatePremium(request);
        assertEquals(calculatePremium, new BigDecimal(9));
    }
