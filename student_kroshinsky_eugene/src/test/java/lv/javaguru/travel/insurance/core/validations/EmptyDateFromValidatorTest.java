...
class EmptyDateFromValidatorTest {
    ...
    @Test
    void validateNullDateFromTest() {
        when(request.getAgreementDateFrom()).thenReturn(null);
        when(validationErrorFactory.createValidationError("ERROR_CODE_3")).thenReturn(expectedError);
        Optional<ValidationError> validationError = validator.validate(request);
        assertTrue(validationError.isPresent());
        assertEquals(expectedError, validationError.get());
    }
    @Test
    void validateNoErrorsDateFromTest() {
        when(request.getAgreementDateFrom()).thenReturn(createDate("24.11.2030"));
        Optional<ValidationError> validationError = validator.validate(request);
