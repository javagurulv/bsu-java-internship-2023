...
class TravelCalculateEmptyLimitValidatorTest {
    ...
    @InjectMocks TravelCalculateEmptyLimitValidator validator;
    @Test
    ...
    void validateErrorTest() {
        ...
        when(request.getSelectedRisks()).thenReturn(List.of("TRAVEL_MEDICAL"));
        when(validationErrorFactory.createValidationError(anyString())).thenReturn(validationError);
        Optional<ValidationError> result = validator.validate(request);
